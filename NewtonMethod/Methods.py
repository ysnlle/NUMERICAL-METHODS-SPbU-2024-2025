...
def LUP(A, b):
     M, P = A, A.E()
     s, c = M.dim['str'], M.dim['col']

     for i in range(c):
          el = 0
          m = i
          for j in range(s):
               if abs(el) < abs(M.matrix[j][i]):
                    m = j
                    el = M.matrix[i][m]

          temp = M.matrix[m]
          M.matrix[m] = M.matrix[i]
          M.matrix[i] = temp

          temp = P.matrix[m]
          P.matrix[m] = P.matrix[i]
          P.matrix[i] = temp
     ...
     #Ly = Pb Ux = y
     Pb = P * b
     y = Matrix([[0] for i in range(b.dim['str'])])
     ...
     x = Matrix([[0] for i in range(b.dim['str'])])
