...
class Matrix:
    ...
    def PrintM(self, precision=16, width=25):
        """Вывод матрицы с высокой точностью без обнуления малых значений"""
        for row in self.matrix:
            formatted_row = []
            for elem in row:
                # Форматирование с 16 знаками после запятой
                if isinstance(elem, (int, float)):
                    formatted_elem = f"{elem:>{width}.{precision}f}"
                else:
                    formatted_elem = f"{str(elem):>{width}}"
